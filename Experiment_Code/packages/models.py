...
import numpy as np
from math import pi
from numpy import absolute, exp, sign, array
...
def cohen_avoid2(args, dphi, s, beta, dtheta, dpsi):
    ...
    ddphi = -b1 * dphi - dpsi * k1 * exp(-c5 * absolute(dpsi)) * (1 - exp(-c6 * max(0, dtheta))) * sigmoid
    ds = b2 * (ps - s) + sign(beta) * dpsi * k2 * exp(-c7 * absolute(dpsi)) * (1 - exp(-c8 * max(0, dtheta))) * sigmoid
    return {'ds': ds, 'ddphi': ddphi}

def cohen_avoid3(args, beta, dtheta, dpsi):
    k1, c5, c6, k2, c7, c8 = args['k1'], args['c5'], args['c6'], args['k2'], args['c7'], args['c8']
    sigmoid = 1 / (1 + exp(20 * (absolute(beta) - 1.3)))
    dds = k2 * sign(beta) * dpsi * exp(-c7 * absolute(dpsi)) * (1 - exp(-c8 * max(0, dtheta))) * sigmoid
    ddphi = -k1 * dpsi * exp(-c5 * absolute(dpsi)) * (1 - exp(-c6 * max(0, dtheta))) * sigmoid
    return {'dds': dds, 'ddphi': ddphi}

def cohen_avoid4(args, beta, dtheta, dpsi):
    k1, c5, c6, k2, c7, c8 = args['k1'], args['c5'], args['c6'], args['k2'], args['c7'], args['c8']
    sigmoid = 1 / (1 + exp(20 * (absolute(beta) - 1.3)))
    dds = k2 * sign(beta) * sign(dpsi) * exp(-c7 * absolute(dpsi)) * (1 - exp(-c8 * max(0, dtheta))) * sigmoid
    ddphi = -k1 * sign(dpsi) * exp(-c5 * absolute(dpsi)) * (1 - exp(-c6 * max(0, dtheta))) * sigmoid
    return {'dds': dds, 'ddphi': ddphi}
...
def cohen_avoid4_thres(args, beta, dtheta, dpsi):
    ...
    dpsi = sign(dpsi) * max(abs(dpsi) - thres, 0)
    sigmoid = 1 / (1 + exp(20 * (absolute(beta) - 1.3)))
    dds = k2 * sign(beta) * sign(dpsi) * exp(-c7 * absolute(dpsi)) * (1 - exp(-c8 * max(0, dtheta))) * sigmoid
    ddphi = -k1 * sign(dpsi) * exp(-c5 * absolute(dpsi)) * (1 - exp(-c6 * max(0, dtheta))) * sigmoid
    return {'dds': dds, 'ddphi': ddphi}

'''
Created by Jiuyang Bai on 11/27/2020
baijiuyang@hotmail.com
This is an experiment on approaching to a stationary goal, in which
participant first walk behind a pacing pole with speed 0.5 and 1.4 m/s then the leader
will disappear and goal will appear at different distance (3 and 6 m) and
directions (+-10, +-15, and +- 20 degrees).
and at different speed. There are 10 repetitions (5 positive 5 negative angle)
in each condition. There are 10 freewalk trials, in which they just walk to
a goal directly ahead. There are 3 * 2 * 2 * 10 + 10 = 130 trials in total.
'''
import random
import os.path
import math
...
def masterLoop(num):
    if conditions[ii][0] < 1 or not DATA_COLLECT:
        ...
        recorded = True
    # conditions: [i_trial, s0, d0, angle]
    run_trial(int(conditions[ii][0]), conditions[ii][1], conditions[ii][2], conditions[ii][3], recorded)
...
def run_trial(i_trial, s0, d0, angle, recorded):
    ...
    if recorded and recording:
        ...
        data = [trial_time, cur_pos[0], cur_pos[1], cur_pos[2], cur_rot[0], cur_rot[1], cur_rot[2],
                goal[0], goal[2], models['goalPole'].getVisible()]
    ...
    if stage == 'ready':
        ...
        if print_flag:
            print_flag = False
            print('>>> Start Trial ' + str(i_trial) + ': ' + ', '.join([str(s0), str(d0), str(angle)]))
            # Compute goal pole position and pacePole velocity for later use
            ang = angle / 360 * 2 * math.pi
            subj_goal_unit = rotate(DIAGONAL_UNIT[i_trial % 2], ang) # unit vector pointing from subj to goal
            p_goal = [h + d * RAMP_DIST + i * d0 for i, h, d in zip(subj_goal_unit, HOME_POLE[i_trial % 2], DIAGONAL_UNIT[i_trial % 2])]
            p_pace = [h + d * PACE_DIST for h, d in zip(HOME_POLE[i_trial % 2], DIAGONAL_UNIT[i_trial % 2])]
...
if __name__ == "__main__":
    ...
    models['orientPole'] = viz.add(os.path.abspath(os.path.join(MODEL_DIR, 'pole_red.osgb')))
    models['goalPole'] = viz.add(os.path.abspath(os.path.join(MODEL_DIR, 'pole_green.osgb')))
    models['pacePole'] = viz.add(os.path.abspath(os.path.join(MODEL_DIR, 'pole_yellow.osgb')))  
    ...
    inputFile = os.path.abspath(os.path.join(INPUT_DIR, 'Approach_subj' + subject + '.csv'))

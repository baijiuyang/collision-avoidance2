...
def run_trial(i_trial, angle, speed, recorded):
    ...
    if stage == 'ready':
        ...
        if print_flag:
            print_flag = False
            print('>>> Start Trial ' + str(i_trial) + ': ' + str(angle) + ', ' + str(speed))
    ...
    # Obstacle appears    
        ...
        if overTheLine(cur_pos, -END_DIST, i_trial):
            # Write data to disk
            if recorded:
                filename = os.path.abspath(os.path.join(OUTPUT_DIR, 'Subj' + subject + '.csv'))
...
if __name__ == "__main__":
    ...
    models['obstPole'] = viz.add(os.path.abspath(os.path.join(MODEL_DIR, 'pole_red.osgb')))  
    ...
    inputFile = os.path.abspath(os.path.join(INPUT_DIR, 'Subj' + subject + '.csv'))
